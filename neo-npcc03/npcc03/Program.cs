...
namespace npcc03
{
    ...
    class Program
    {
        public const string ProgramName = "npcc03 - NEO Persistable Classes (NPC) Platform 3.0 Compiler";
        ...
        static void Main(string[] args)
        {
            ...
            foreach (FileInfo fi in di.GetFiles("*.dll"))
            {
                ...
                for (int classIndex = 0; classIndex < ctx.listClassInfo.Count; classIndex++)
                {
                    ...
                    {
                        success = GenCodeNeo03.GenerateCodeLevel0Basic(ctx, classIndex);
                    ...
                    if (listClassInterfaces.Count == 1)
                    {
                        success = GenCodeNeo03.GenerateCodeLevel1Managed(ctx, classIndex);
                    ...
                    if (listClassInterfaces.Count == 1)
                    {
                        success = GenCodeNeo03.GenerateCodeLevel2Persistable(ctx, classIndex);
                    ...
                    if (listClassInterfaces.Count == 1)
                    {
                        success = GenCodeNeo03.GenerateCodeLevel3Deletable(ctx, classIndex);
                    ...
                    if (listClassInterfaces.Count == 1)
                    {
                        success = GenCodeNeo03.GenerateCodeLevel4CollectibleExt0(ctx, classIndex);
                        ...
                        success = GenCodeNeo03.GenerateCodeLevel4CollectibleExt2(ctx, classIndex);
                    ...
                    if (listClassInterfaces.Count >= 1)
                    {
                        success = GenCodeNeo03.GenerateCodeCustomMethods(ctx, classIndex, listClassInterfaces);
